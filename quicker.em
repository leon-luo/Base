...
macro get_header_filename_extension(num)
{
...
macro get_source_filename_extension(num)
{
	h_buf = NewBuf("extension")
	if (h_buf == hNil)
		stop
...
function is_header_file()
{
	...
	num = 0
	//Msg("1num=@num@")
	extension = get_header_filename_extension(num)
	//Msg("2num=@num@")
	//Msg("extension=@extension@")
	while(index < num)
	{
		cur_ext = GetBufLine(extension, index)
	...
	index = 0
	num = 0
	extension = get_source_filename_extension(num)
	while(index < num)
	{
		cur_ext = GetBufLine(extension, index)
		ret = isFileType(curOpenFileName, cur_ext)
	...
	while(index < index_cpp_end)
	{
		...
		ret = isFileType(curOpenFileName, curExt)
		if(True == ret) // 匹配成功
		{
