...
macro insert_file_header(hbuf, ln, content_str)
{
	...
	{
		msg("insert_file_header()::0==strlen(\"@content_str@\") return")
		return
	}
...
macro get_curr_line_str()
{
	...
	hbuf = GetWndBuf(hwnd)
	sel = GetWndSel(hwnd)
	line_num = sel.ichFirst
	line_str = GetBufLine(hbuf, line_num)
	msg("get_curr_line_str() :: line_str=[@line_str@]")
	return line_str
}
...
macro get_pretty_code_left_format()
{
	...
	retract_line = strmid(line_str, 0, ich) # retract       //缩进行
	
	data.alignment = alignment_line
	data.retract = retract_line
	msg("get_pretty_code_left_format() :: line_str=[@line_str@]")
	return data
}
...
macro auto_expand()
{
	...
	left_format = get_pretty_code_left_format()
	
	alignment_line = left_format.alignment//strmid(line_str, 0, ich)               //对齐行
	retract_line = left_format.retract//strmid(line_str, 0, ich) # retract       //缩进行
...
function get_curr_file_type()
{
	...
	msg("get_curr_file_type() : get_curr_open_file_absolute_path()=[@open_file@]")
