...
macro insert_do_while()
{
	...
	if(sel.lnFirst == sel.lnLast && sel.ichFirst == sel.ichLim)
	{
		PutBufLine(handle, line_num+1, "@retract@#")
	}
	msg(cat("sel.lnLast=", sel.lnLast))
	msg(cat("val.nLineCount=", val.nLineCount))
	line = sel.lnLast + val.nLineCount
	msg("line={@line@}")
	msg("line_num={@line_num@}")
	PutBufLine(handle, line, "@alignment@}while ( # );")
	//SetBufIns (hbuf, line, strlen(temp_left)+8)
