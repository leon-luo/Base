...
macro get_system_time()
{
	...
	SysTime.Date=cat(SysTime.Year, year_unit)
	SysTime.Date=cat(SysTime.Date, SysTime.Month)
	SysTime.Date=cat(SysTime.Date, month_unit)
	SysTime.Date=cat(SysTime.Date, SysTime.Day)
	SysTime.Date=cat(SysTime.Date, day_unit)
...
macro expand_proc_chinese(author_name,wordinfo,retract_line,alignment_line,nVer,ln,sel)
{
	commend_str = wordinfo.word
	hwnd = GetCurrentWnd()
	if (hwnd == 0)
...
macro insert_file_header_english(hbuf, ln, name_str, content_str)
{
	...
	InsBufLine(hbuf, ln + 2,  "@copyright_str@")
	...
	InsBufLine(hbuf, ln + 8,  "  Created       : @curr_Time@")
...
函数说明信息头
*/
macro function_head_comment(hbuf, ln, curr_Func, author_name, newFunc)
{
	iIns = 0
	is_english = test_language_is_english()
	...
	if(True == is_english)
	{
		...
		modification_details_str = "修改内容"
		new_function_str = "新生成函数"
...
macro insert_history_content(hbuf, ln, iHostoryCount)
{
	...
	InsBufLine(hbuf, ln, "")
	InsBufLine(hbuf, ln + 1, "  @iHostoryCount@.@data_str@: @curr_time_str@")

	InsBufLine(hbuf, ln + 2, autor_str)
