...
function get_curr_file_type()
{
	...
	while(index < num)
	{
		...
		ret = isFileType(curr_open_file_name, cur_ext)
		if(True == ret)
		{
			file_type = hxx
			return file_type
...
function get_curr_open_filename()
{
	handle = GetCurrentBuf()
	if (handle == hNil)// empty buffer
		stop

	filename = GetBufName(handle)
	return filename
}
...
macro switch_cpp_hpp()
{
	...
	filename = get_curr_open_filename()
	Msg(cat("filename:", filename))
	...
	Msg(cat("current opened filename lenght:", curOpenFileNameLen))
	...
	while(index < index_replace_end)
	{
		...
		Msg("destExt=@destExt@")
		Msg("destFileName=@destFileName@")
		// 尝试当前目标扩展名是否能够打开
		destFilePath = cat(destFileName, destExt) // 添加文件名之后的后缀名（包括:绝对路径\文件名.扩展名）
		Msg("destFilePath=@destFilePath@")
		// 调试
